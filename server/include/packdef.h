...
struct FileInfo
{
    ...
    FileInfo():fid(0),size(0),fileFd(0), pos(0),timestamp(0){
...
struct STRU_GET_FILE_INFO_RS{
    ...
    int count;
    //文件信息数组
    STRU_FILE_INFO fileInfo[];//柔性数组 在使用时通过count来确定数组大小，通过malloc申请空间，但是不会走构造函数需要手动调用init进行初始化
};
...
struct STRUCT_QUICK_UPLOAD_RS{
    STRUCT_QUICK_UPLOAD_RS():type(_DEF_PACK_QUICK_UPLOAD_RS),timestamp(0),userid(0),result(1)
    {
...
//收到回复 就刷新分享列表
struct STRUCT_SHARE_FILE_RS
{
    STRUCT_SHARE_FILE_RS():type(_DEF_PACK_SHARE_FILE_RS),result(0){}
    PackType type;
    int result;
};
...
//分享文件信息 分享文件的列表 文件：名字 大小 分享时间 链接（密码 todo）
...
{
    void init(){
           type=_DEF_PACK_MY_SHARE_RS; int iteCount=0;
    }
    PackType type;
    int iteCount;
